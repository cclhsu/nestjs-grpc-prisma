...
import { MyGrpcModule } from './grpc.module';
...
//   const app: INestApplication<any> = await NestFactory.create(MyGrpcModule, {
...
export async function bootstrap() {
  try {
    ...
    httpApp.connectMicroservice(grpcOptions);
    httpsApp?.connectMicroservice(grpcOptions);
    ...
    // // Register the TransformInterceptor as a global interceptor
    // httpApp.useGlobalInterceptors(transformInterceptor);
    // httpsApp.useGlobalInterceptors(transformInterceptor);
    ...
    // // Register the PlainToClassInterceptor as a global interceptor
    // httpApp.useGlobalInterceptors(plainToClassInterceptor);
    // httpsApp.useGlobalInterceptors(plainToClassInterceptor);
    ...
    httpApp.listen(httpOptions.port, () => {
      ...
      // console.log(
      //   `Grpc Playground is running on: http://${httpOptions.host}:${httpOptions.port}/grpc`,
      // );
    });
    ...
    if (httpsEnabled === true && httpsApp !== undefined) {
      ...
      httpsApp?.listen(httpsOptions.port, () => {
        ...
        // console.log(
        //   `Grpc Playground is running on: https://${httpsOptions.host}:${httpsOptions.port}/grpc`,
        // );
      });
    }
  } catch (error) {
